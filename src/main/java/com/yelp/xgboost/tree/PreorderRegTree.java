...
/**
 * Memory-compact and cache efficient implementation of a regression tree. The tree is stored as an
 * int array where each node is represented as a block of 3 ints:
 *
 * <p>-------------------------------------------------------- 1 | Split condition / Leaf Value (32
 * bits) | -------------------------------------------------------- 2 | Right Child Offset (31 bits)
 * | False is left (1 bit) | (Zero iff node is leaf)
 * -------------------------------------------------------- 3 | Feature Index (31 bits) | Default is
 * right (1 bit) | (Original ID iff node is leaf)
 * --------------------------------------------------------
 *
 * <p>Notes on terms: - The "left child" is considered the child immediately next in the node array
 * - The "right child" is packed some unknown length after the parent in the node array - In the
 * loaded XGboost tree, "left" is considered the case where the condition at the node is found to be
 * true
 *
 * <p>Trees output by XGboost have their nodes indexed as if by breadth first traversal, leading to
 * child nodes being increasingly far in memory from the parent node. Using each node's cover of the
 * training data, we repack the tree with depth first pre-order in order of which child node saw the
 * most training data. Assuming that the distribution of training data is roughly the same as the
 * distribution seen online, this effectively guarantees that at any point in tree traversal the
 * most common path is immediately adjacent in memory and reached as the if rather than the else
 ...
 *
 * <p>Design Specifics / Limitations: - XGBoost trees are indexed breadth-first from the root with
 * the left child always immediately adjacent to the right child. We repack the tree with respect to
 * node coverage giving the property that frequently evaluated paths are compact in memory, in
 * theory encouraging cache hits. It is always the case that a parent node has a higher cover than a
 * child node, but not that a node has a higher cover than a child to a sibling node. The trade-off
 * is that rare routes almost guarantee a cache miss.
 *
 * <p>- Array indices are 32 bit integers whose max value is JVM specific but always &lt; Integer
 * .MAX_VALUE. Additionally, since tree nodes are packed with stride BLOCK_SIZE, this upper bound is
 * effectively further reduced by log2(3). We overcome this limitation by using an otherwise wasted
 * sign bit of the int for determining conditional direction and further by considering the child
 * offset rather than outright index, which in a complete tree has an upper bound of exp(2,
 * node_depth). Therefore we require 2 fewer bits than a simple int indexing.
 *
 * <p>- Feature index is allocated 31 bits. This implementation can only support 2^31 features.
 * Unlike with node indexes, we use longs for our sparse feature vectors and therefore could only
...
public class PreorderRegTree extends AbstractRegTree {
  ...
  protected int getNextNode(int index, FVec feat) {
    Float fvalue = feat.fvalue(nodes[index + 2] >>> 1);
    ...
    if ((fvalue < Float.intBitsToFloat(nodes[index])) != ((nodes[index + 1] & 0x1) == 1)) {
      /*
